...
class Turtle:
    ...
    def __init__(self, _linewidth, _materialindex):
        ...
        self.mat = self.mat @ Matrix.Rotation(radians(270), 4, 'Y') 
    ...
    def turn(self, angle_degrees):
        self.mat = self.mat @ Matrix.Rotation(radians(angle_degrees), 4, 'Z')
    def pitch(self, angle_degrees):
        self.mat = self.mat @ Matrix.Rotation(radians(angle_degrees), 4, 'Y')
    def roll(self, angle_degrees):
        self.mat = self.mat @ Matrix.Rotation(radians(angle_degrees), 4, 'X')
...
class DrawingTurtle(Turtle):
    ...
    def create_default_internode_mesh(self, vertex_count):
        ...
        bpy.ops.mesh.select_all(action='SELECT')
        bpy.ops.transform.rotate(value=radians(90), orient_axis='Y') #axis=(0, 1, 0))
    ...
    def create_default_node_mesh(self, _subdivisions=1):
        ...
        bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=_subdivisions, radius=0.5)#size=0.5)
