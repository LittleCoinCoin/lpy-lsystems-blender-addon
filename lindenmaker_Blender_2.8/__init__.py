bl_info = {
    ...
    "version": (1, 1),
    "blender": (2, 83, 0),
    "location": "View3D > Add > Mesh > L-System Object",
...
class LPY_PT_lindenmaker(bpy.types.Panel):
    """Lindenmaker Panel"""
    bl_label = "Lindenmaker"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'#'TOOLS'
    bl_context = "objectmode"
    bl_category = "panel.lindenmaker"
    ...
    def draw(self, context):
        ...
        op_lindenmaker = layout.operator(OBJECT_OT_lindenmaker.bl_idname, icon='OUTLINER_OB_MESH')
        ...
        if context.scene.section_lstring_expanded is True:
            ...
            op_clear = boxcol.operator(OBJECT_OT_lindenmaker.bl_idname,
            ...
            op_produce_step = boxcol.operator(OBJECT_OT_lindenmaker.bl_idname,
            ...
            boxcol.label(text="L-string for Production (after " + str(context.scene.number_production_steps_done) + " steps):")
            ...
            boxcol.label(text="Homomorphism (for Interpretation):")
            boxcol.prop(context.scene, "lstring_for_interpretation", text="")
            
            # button to do interpretation only (no production)
            op_interpret = boxcol.operator(OBJECT_OT_lindenmaker.bl_idname,
            ...
            op_interpret_step = boxcol.operator(OBJECT_OT_lindenmaker.bl_idname,
...
class OBJECT_OT_lindenmaker(bpy.types.Operator):
    ...
    lstring_production_mode = bpy.props.EnumProperty(name="L-string Production Mode",
    ...
    bool_clear_lstring =bpy.props.BoolProperty(name="Clear L-string",
                                                description="Clear current L-string", 
                                                default=True)
    bool_interpret_lstring = bpy.props.BoolProperty(name="Interpret L-string",
...
def menu_func(self, context):
    self.layout.operator(OBJECT_OT_lindenmaker.bl_idname, icon='PLUGIN')

classes = (LPY_PT_lindenmaker, OBJECT_OT_lindenmaker)
...
def delete_hierarchy(obj):
    ...
    [bpy.data.objects[name].select_set(True) for name in objnames]
    bpy.ops.object.delete()
